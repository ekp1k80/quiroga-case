"use client";

import React, { useEffect, useRef, useState } from "react";
import styled from "styled-components";
import AndroidLockScreen from "@/components/AndroidLockscreenSim";
import AndroidIncomingCall from "@/components/AndroidIncomingCall";
import { useSoundUnlock } from "@/hooks/useSoundUnlock";
import { preloadBufferSound } from "@/components/playBufferSound";
import AndroidInCall from "@/components/AndroidInCall";
import PlayAudio from "@/components/PlayAudio";
import AndroidLocked from "@/components/AndroidLocked";
import ViewportWrapper from "@/components/ViewportWrapper";
import AudioPlayer from "@/components/AudioPlayer";
import { usePublicAudio } from "@/hooks/usePublicAudio";
import { AudioVizConfig } from "@/data/packs";
import { useFullscreen } from "@/hooks/useFullscreen";
import CarTravel from "@/components/CarTravelSprite";
import PulsingText from "@/components/PulsingText";
import WiFiRouterHackScene from "@/components/WiFiRouterHackScene";

type Scene = "start" | "idle1" | "idle2" | "idle3" | "idle4" | "idle3" | "idle4" | "idle5" | "idle6" | "idle7" | "idle8" | "llegadaAudio" | "elHorrorAudio" | "sofiaAudio" | "camaraUI" | "camaraAudio" | "done";

const viz = {
	gain: 1.6,
	gate: 0.1,
	gateSoft: 0.11,
	minBinFrac: 0.08
}

export default function PhoneScenePage() {
  const [scene, setScene] = useState<Scene>("start");
  const { src, blob, } = usePublicAudio("LaCasaDeHector.mp3");
  const { src: srcElHorror, blob: blobElHorror, } = usePublicAudio("Casa_Hector_El_Horror.mp3");
  const { src: srcSofia, blob: blobSofia, } = usePublicAudio("LaCasaDeHectorSofia.mp3");
  const { src: srcLaCamara, blob: blobLaCamara, } = usePublicAudio("LaCasaHectorLaCamara.mp3");
	const { enter } = useFullscreen();
  const { unlockNow, audioContextRef } = useSoundUnlock();


  const mainAudio = "ViajeCentragolo.mp3";

  const handleStart = async () => {
		// await enter()
    const ok = await unlockNow();
    // Preload notificación (WebAudio) para que salga instant
    if (ok && audioContextRef.current) {
      try {
        await preloadBufferSound(audioContextRef.current, mainAudio);
      } catch {}
    }
    // No pre-cargamos el ringtone con WebAudio porque lo reproducimos con <audio loop>,
    // pero podrías hacer un fetch “calentón” si querés.
    setScene("idle1");
  };

  return (
    <Root>
      {scene === "start" && (
        <StartOverlay onClick={handleStart}>
          <StartCard>
            <StartTitle>Tap para desbloquear</StartTitle>
            <StartHint>(toca la pantalla)</StartHint>
          </StartCard>
        </StartOverlay>
      )}
			
      {(scene === "idle1") && (
        <AndroidLocked
          black
          fixedTime=""
          timerEnd={1000}
          onEnd={() => { setScene("idle2"); }}
        />
      )}
      
      {(scene === "idle2") && (
        <AndroidLocked
          fixedTime="11:00"
          timerEnd={4000}
          onEnd={() => { setScene("llegadaAudio"); }}
        />
      )}
      
      {(scene === "llegadaAudio") && (
        <ViewportWrapper
					fixedTime="11:00"
          showStatusBar
				>
          <AudioPlayer
            src={src as string}
            blob={blob as Blob}
            title={" "}
            viz={viz as AudioVizConfig}
            showControls={false}
            autoPlay
            barCount={50}
            onEnded={() => { setScene("idle3"); }}
          />
      </ViewportWrapper>
      )}

      {(scene === "idle3") && (
        <AndroidLocked
        black
        fixedTime=""
        timerEnd={1000}
        onEnd={() => { setScene("idle4"); }}
        />
      )}
      
      {(scene === "idle4") && (
        <AndroidLocked
        fixedTime="11:04"
        timerEnd={2000}
        onEnd={() => { setScene("elHorrorAudio"); }}
        />
      )}
      
      {(scene === "elHorrorAudio") && (
        <ViewportWrapper
          fixedTime="11:04"
          showStatusBar
        >
          <AudioPlayer
            src={srcElHorror as string}
            blob={blobElHorror as Blob}
            title={" "}
            viz={viz as AudioVizConfig}
            showControls={false}
            autoPlay
            barCount={50}
            onEnded={() => { setScene("idle5"); }}
          />
      </ViewportWrapper>
      )}

      {(scene === "idle5") && (
        <AndroidLocked
        black
        fixedTime=""
        timerEnd={1000}
        onEnd={() => { setScene("idle6"); }}
        />
      )}
      
      {(scene === "idle6") && (
        <AndroidLocked
        fixedTime="11:15"
        timerEnd={2000}
        onEnd={() => { setScene("sofiaAudio"); }}
        />
      )}
      
      {(scene === "sofiaAudio") && (
        <ViewportWrapper
          fixedTime="11:15"
          showStatusBar
        >
          <AudioPlayer
            src={srcSofia as string}
            blob={blobSofia as Blob}
            title={" "}
            viz={viz as AudioVizConfig}
            showControls={false}
            autoPlay
            barCount={50}
            onEnded={() => { setScene("idle7"); }}
          />
      </ViewportWrapper>
      )}

      {(scene === "idle7") && (
        <AndroidLocked
          black
          fixedTime=""
          timerEnd={1000}
          onEnd={() => { setScene("idle8"); }}
        />
      )}
      
      {(scene === "idle8") && (
        <AndroidLocked
          fixedTime="11:21"
          timerEnd={4000}
          onEnd={() => { setScene("camaraUI"); }}
        />
      )}

      {(
        scene === "camaraUI" && (
          <WiFiRouterHackScene
            fixedTime="11:21"
            onAfter={() => {
              setScene("camaraAudio");
            }}
          />

        )
      )}

       {(scene === "camaraAudio") && (
        <ViewportWrapper
          fixedTime="11:26"
          showStatusBar
        >
          <AudioPlayer
            src={srcLaCamara as string}
            blob={blobLaCamara as Blob}
            title={" "}
            viz={viz as AudioVizConfig}
            showControls={false}
            autoPlay
            barCount={50}
            onEnded={() => { setScene("done"); }}
          />
      </ViewportWrapper>
      )}

      {scene === "done" && (
        <DoneOverlay>
          <DoneText>Fin de la escena (seguí con tu juego acá).</DoneText>
        </DoneOverlay>
      )}
    </Root>
  );
}

/* ===== styles simples del wrapper ===== */

const Root = styled.div`
  height: 100svh;
  width: 100%;
  background: #000;
`;

const StartOverlay = styled.div`
  position: fixed;
  inset: 0;
  display: grid;
  place-items: center;
  background: rgba(0, 0, 0, 0.78);
  z-index: 9999;
  cursor: pointer;
`;

const StartCard = styled.div`
  width: min(340px, 86%);
  border-radius: 18px;
  border: 1px solid rgba(255, 255, 255, 0.14);
  background: rgba(20, 20, 20, 0.92);
  backdrop-filter: blur(10px);
  box-shadow: 0 10px 25px rgba(0, 0, 0, 0.6);
  padding: 18px 16px;
  text-align: center;
`;

const StartTitle = styled.div`
  font-size: 18px;
  font-weight: 800;
  color: #fff;
`;

const StartHint = styled.div`
  margin-top: 8px;
  font-size: 12px;
  opacity: 0.65;
  color: #fff;
`;

const DoneOverlay = styled.div`
  height: 100svh;
  display: grid;
  place-items: center;
`;

const DoneText = styled.div`
  color: #fff;
  opacity: 0.85;
  font-size: 14px;
`;
